...
const LoveMessagePage = () => {
  ...
  const showNotificationMessage = (type: 'error' | 'success', message: string) => {
  ...
  const validateAllFields = (): boolean => {
    ...
    if (!isAnonymous && !senderName.trim()) {
      newErrors.senderName = 'Le nom est obligatoire si vous n\'êtes pas anonyme';
    }
  ...
  const handleFinishOptions = () => {
    ...
    setStep('share');
    showNotificationMessage('success', 'Message créé avec succès!');
  };
  ...
  // Return principal avec notification
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-900 to-pink-900 relative">
          ...
          >
            ...
            <div className="mb-6 sm:mb-8">
              <textarea
                ...
                maxLength={500}
                className={`w-full h-48 sm:h-64 p-4 border-2 rounded-2xl focus:outline-none resize-none text-gray-700 text-sm sm:text-base bg-pink-50/50 ${
                  errors.message ? 'border-red-500 bg-red-50' : 'border-pink-200'
                }`}
              />
              {errors.message && (
                <p className="mt-2 text-red-500 text-xs sm:text-sm flex items-center gap-1">
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full p-3 border-2 rounded-lg focus:border-pink-500 focus:outline-none text-gray-700 text-sm ${
                        errors.senderName ? 'border-red-500 bg-red-50' : 'border-pink-200'
                      }`}
                    />
                    {errors.senderName && (
                      <p className="mt-2 text-red-500 text-xs sm:text-sm flex items-center gap-1">
